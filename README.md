...
Engine-side aggregation (`engine/vector/agg.go`, tables in `ht.go`, partitioned merge in `parthash.go`) folds batches into per-worker local state (`GlobalFloatAgg`, `GroupByInt64`, `GroupByString`) with a single-threaded `Merge` (dense tables fan the merge out past a 64K-entry gate), so there are no shared tables and no locks. NULL values are counted but not summed (SQL semantics). NULL keys form their own group. `Parallel{GlobalAgg,GroupByInt64,GroupByString}` shard resident batches across workers and merge. PG `GROUP BY` pushdown stays the serving route: on the 1M-row bench, pushdown drains 128 rows at about 11.8M scanned rows/s while the engine path scans 1M rows at about 9.4M scanned rows/s, so less wire wins. The engine path exists for future non-PG sources and is parity-checked against PG group-by results cell for cell (nulls, Sel-filtered batches, absolute float tolerance 1e-6; see `postgres/vector_agg_test.go`).

The tables are DuckDB-inspired, ported to what fits Go (source mapping in `ht.go`): a dense perfect-hash array when the key domain is small and known (the Mod path — index IS the reduced key, no hash/probe/compare; power-of-two Mods reduce by mask), an open-addressing linear-probing table with contiguous key/payload arrays otherwise, and dictionary-encoded strings (one map load per row to a dense id, payloads folded in place in a single-cache-line struct). Every path updates payloads in place, removing the old `map[K]groupState` two-probes-per-row (load plus store-back with rehash, write barrier, and value copy). What was deliberately NOT ported: input radix-partitioning and salt-stride probing — measured net-negative at our shapes (a private-table merge fan-out was built, benchmarked at 13ms vs ~6ms sequential, and removed; see `parthash.go`).

Resident 1M-row micro-bench, Apple M2, rows/s (QPX before → after vs in-process DuckDB on the same distribution; every cell is fold plus output materialization on both sides):

| cell | before | after | DuckDB | verdict |
|---|---|---|---|---|
| global | 999M | 3.87B | 1.45B | exceeds (4-accumulator unrolled sum) |
| group-128 (`id%128`) | 56M | 380M | 439M | 1.16× — within 2×, stop |
| group-100K (`id%100000`) | 31M | 307M | 55M | exceeds* — caller-supplied Mod bound, not table-vs-table |
| group-string-1K | 46M | 93M | 230M | 2.49× — stop with proof, see below |
| group-raw-1M (1M distinct) | — | 11.4M | 11.6M | parity |

*group-100K: QPX is handed `Mod=100000` as an a-priori range proof (perfect hash, zero hashing) while DuckDB hashes a generic `GROUP BY (id%100000)` — the win measures that bound, not the table. DuckDB's own 55M here vs 439M on the 128-cell flags the same expression/partitioning artifact.

Remaining gaps, named honestly: group-128's 1.16× is Go bounds checks plus generic addressing in the fold loop vs DuckDB's unchecked C++ inner loop (profile: loop/load overhead, hashing gone). String-1K's 2.49× is the Go runtime-map lookup floor: hashing each row once plus probe plus key compare in un-inlinable runtime code, against DuckDB's custom C++ table with inlined hashing — a lookup-only (no fold) pass already costs 10.9ms vs DuckDB's 4.7ms total, so no fold design in Go can reach parity; the dict design reduced the fold itself to ~zero. Per-structure micro-benchmarks (`engine/vector/ht_bench_test.go`) isolate each table; serving-shape benches live in `bench/`.

Worker scaling on live PG scans is flat 1 to 16 (single-connection scan floor, see below). On resident batches the parallel fold scales 379M (W1) to 714M (W2) to 1325M (W4) to 1590M (W8) to 1633M (W16) rows/s: no contention by construction, core-bound past 8.
